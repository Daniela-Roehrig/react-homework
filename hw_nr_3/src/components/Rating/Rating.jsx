...
const Rating = () => {
  ...
  return (
    <div className={styles.ratingContaine}>
      <img
        src={ratingList[ratingValue - 1] || ratingList[0]} 
        alt="Rating"
        className={styles.ratingImage}
      />
      <div className={styles.buttonsContainer}>
        <button onClick={() => handleRating(1)}>Плохо</button>
        <button onClick={() => handleRating(2)}>Приемлемо</button>
        <button onClick={() => handleRating(3)}>Хорошо</button>
        <button onClick={() => handleRating(4)}>Отлично</button>
      </div>
      <p className={styles.ratingPara}>Вы выбрали рейтинг: {ratingValue}</p>
    </div>
  );
};
